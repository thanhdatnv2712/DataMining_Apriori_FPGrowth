    ...
    minimum_support_threshold( minimum_support_threshold )
{
    ...
    #ifdef DEBUG
    if (frequency_by_item.size() > 0) {
        // print table Item and frequency
        std::cout << "Scan the transactions counting the frequency of each item" << std::endl;
        for (auto frequency_by_item_iter : frequency_by_item) {
            std::cout << frequency_by_item_iter.first << ": " << frequency_by_item_iter.second << std::endl;
        }
        std::cout << std::endl;
    }
    #endif
